...
//        aluno.setTurma(turma);
    ...
    /*
     * Inner Action chamada automaticamente pelo framework
     * para obter os requisitos para exibir a tela de
     * atualização.
     */
    @Override
    protected void restoreObject() {
        aluno = Dao.get(Aluno.class, aluno.getId());
        output.setValue("aluno", aluno);

        List turmas = Dao.list(Turma.class, "", "codigo asc");
        int turmaId = aluno.getTurma().getId();

        output.setValue("turmas", turmas);
        output.setValue("turma", turmaId);
    }

    /*
     * Salva o aluno editado no banco de dados
     */
    public String update() {
        Dao.updateTransaction(aluno);
        return SUCCESS;
    }
