...
public class DataModelMigrationController {
	...
	public static void execute() throws Exception {
		...
		String dataModelClassName = Params.get("APP_DATA_MODEL_CLASS");
		boolean avoidExecutions = !Params.get("SCHEMA_GENERATION_TYPE").equals("none");
		...
		try{
			...
			System.out.println("[fpg] You didn't define a "+VersionedDataModel.class.getName()+" correctly. APP_DATA_MODEL_CLASS="+Params.get("APP_DATA_MODEL_CLASS"));
			throw ex;
		}
		...
		File versionsMigrationDir = new File(Params.get("CLASSES_REAL_PATH")+"/migration/versions");
		if(!versionsMigrationDir.exists()) {
			...
			if(Params.devMode() && pastVersionsDir.exists() && pastVersionsDir.isDirectory() && getVersionNum(appModel.getVersion()) < getVersionNumOf(migrationFiles.get(0).getName())){
				System.out.println("[fpg] DMMC: Loading migrations on 'past' dir ...");

				ArrayList pastFiles = FileUtil.listFilesFromDirectory(pastVersionsDir,true);
			...
			if(avoidExecutions){
				String infoLog = "SCHEMA_GENERATION_TYPE="+Params.get("SCHEMA_GENERATION_TYPE")+", migrations skipped. To run you need to set to 'none'!";
				System.out.println("[fpg] DMMC: "+infoLog);
				logTxt.append(infoLog).append("\n");
			}

			double appVersionNum = getVersionNum(appModel.getVersion());
			int success = 0;
			int skipped = 0;
			...
			if(migrationFiles.size()>0){
				...
				if(getVersionNumOf(migrationFiles.get(migrationFiles.size()-1).getName()) > appVersionNum ){
					...
					if(exCaused!=null){
						DefaultExceptionLogger.getInstance().execute(exCaused);
			...
			if(newVersion!=null){
				...
				if(avoidExecutions){
					...
					if(     !Params.get("DEPLOY_MODE").equals("production")
						 || !Is.empty(Params.get("APP_BUILD_ID"))
						 ||  Params.get("LOG_RESTART").equals("true")
					){
				...
				// clean build info.
				if(!Is.empty(Params.get("APP_BUILD_ID"))){
					HashMap<String,String> contentMap = new HashMap();
					contentMap.put("<param.*\\bname=\"APP_BUILD_ID\".*?>","");
					String appParamsFilePath = The.concat(Params.get("CLASSES_PATH"),"/conf/app-params.xml");
					FileUtil.putStrings(contentMap, appParamsFilePath, appParamsFilePath, true);

					if(Params.get("DEPLOY_MODE").equals("production")){
						// save info to build tools.
						String finalVersion = newVersion!=null? newVersion: (!Is.empty(oldVersion)?oldVersion:"0");
						File initDir = new File(Params.get("WEB_REAL_PATH")+"/init");
						if(!initDir.exists()){
							//noinspection ResultOfMethodCallIgnored
							initDir.mkdir();
						}
						if(initDir.exists() && initDir.isDirectory()){
							FileUtil.createTextFile(finalVersion, Params.get("WEB_REAL_PATH")+"/init/DMMC_VERSION.info");
						}else{
	...
	private static ArrayList sortByVersion(ArrayList list) {
		Collections.sort(list, new Comparator<File>() {
			...
			public int compare(File f1, File f2) {
				return getVersionNumOf(f1.getName()) > getVersionNumOf(f2.getName())? 1 : -1;
			}
		});
		return list;
	}
	...
	private static double getVersionNum(String strVersion){
		if(Is.empty(strVersion)){
			return 0.0d;
		}else{
			return Double.valueOf(strVersion.replace("_","."));
		}
	}
}